...
TcpSocketBase::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::TcpSocketBase")
    .SetParent<TcpSocket> ()
    .AddTraceSource ("AllowedWindow",
                     "The effective Window used by TCP transmissor",
                     MakeTraceSourceAccessor (&TcpSocketBase::m_allowedWnd))
    // in test... so for now use GlobalFixedTcpWindowSize
    .AddAttribute ("FixedTcpWindowSize",
                   "0 disable this feature",
                   UintegerValue (0),
                   MakeUintegerAccessor (&TcpSocketBase::m_fixedTcpWindowSize),
...
TcpSocketBase::Recv (uint32_t maxSize, uint32_t flags)
{
  ...
  // talau: outPacket will have only data, without any headers
                          ...
                          Ptr<Ipv4Interface> incomingInterface)
{
  ...
  if (Node::UseNGWA ()) 
    {
      m_ngwa_bandwidth = header.GetOptions ();
      //std::cout << "options: " << m_ngwa_bandwidth << std::endl;
      //std::cout << "options: " << header.GetOptions() << std::endl;
    }
...
TcpSocketBase::ReceivedAck (Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
  ...
  if (0 == (tcpHeader.GetFlags () & TcpHeader::ACK))
      ...
      if (tcpHeader.GetAckNumber () < m_nextTxSequence)
        {
          ...
          if (m_dupAckCount >= 3)
            {
              m_node->reportTripleAcks.insert(std::pair<uint16_t, uint16_t> (Simulator::Now ().GetSeconds (), m_dupAckCount / 3));
              NS_LOG_LOGIC ("Triples Dupack, number = " << m_dupAckCount / 3);
            }
        }
    ...
    { // Case 3: New ACK, reset m_dupAckCount and update m_txBuffer
      ...
      m_dupAckCount = 0;
      NS_LOG_LOGIC ("Dupack count " << m_dupAckCount);
    }
...
{
  NS_LOG_FUNCTION (this);
  return m_nextTxSequence - m_txBuffer.HeadSequence ();
}
...
{
  NS_LOG_FUNCTION (this);
  return m_highTxMark - m_txBuffer.HeadSequence ();
}
...
TcpSocketBase::AvailableWindow ()
{
  ...
  ////m_allowedWnd = (win < unack) ? 0 : (win - unack);

  // try with this:
  m_allowedWnd = win;

  //std::cout << "(WINDOW-INFO) min(Wa,Wc) = " << win << "; allowed window = " << m_allowedWnd << std::endl;
  NS_LOG_LOGIC ("UnAckCount=" << unack << ", Win=" << win);
  return (win < unack) ? 0 : (win - unack);
}

void
TcpSocketBase::NGWA_filter_bandwidth (uint32_t w)
{
  float alfa = 0.3;
  //float alfa = 0.1;

  m_ngwa_avgbandwidth = (1 - alfa) * m_ngwa_avgbandwidth + alfa * w;
  //std::cout << "ngwa_filter: " << m_ngwa_avgbandwidth << std::endl;
}

uint32_t
TcpSocketBase::DoNGWAModeWindowSize (int mode)
{
  uint32_t max = 0xffff;
  uint32_t awnd = std::min (m_rxBuffer.MaxBufferSize () - m_rxBuffer.Size (), max);

  /*
  std::cout << "(NGWA awnd) " << awnd << std::endl;

  if (awnd > m_ngwa_bandwidth)
    {
      std::cout << awnd << " > " << m_ngwa_bandwidth << std::endl;
    }
  */
  ...
  switch (mode) 
    ...
    case 1: // new-reno
      am = USE_ONLY_OPTION;
      break;
      // mode #2 ngwa mode with: suavisation
    case 2:
      am = SUAVISE_OPTION;
      break;
      // mode #3 ngwa mode with: Rfc793 (so without cwnd)
    case 3: // rfc793
      am = USE_ONLY_OPTION;
      break;
      // mode #4 ngwa mode with: Rfc793; suavisation
    case 4:
      am = SUAVISE_OPTION;
      break;
      // mode #5 ngwa with: flow in queue; cwnd
    case 5: // use flow; new-reno
      am = USE_ONLY_OPTION;
      break;
      // mode #6 ngwa with: flow in queue; cwnd; suavisation
    case 6:
      am = SUAVISE_OPTION;
      break;
    case 7: // use flow; rfc793
      am = USE_ONLY_OPTION;
      break;
      // mode #6 ngwa with: flow in queue; cwnd; suavisation
    case 8:
      ...
      break;
    default:
      NS_ABORT_MSG ("NGWA-MODE: Fatal, NGWA mode is invalid.");
      break;
    }

  switch (am)
    {
    case USE_ONLY_OPTION:
      //std::cout << "(NGWA RETURNS) " << std::min (awnd, m_ngwa_bandwidth) << std::endl;
      return std::min (awnd, m_ngwa_bandwidth);
      break;
    case SUAVISE_OPTION:
      //std::cout << "(NGWA RETURNS) " << std::min (awnd, m_ngwa_avgbandwidth) << std::endl;
      NGWA_filter_bandwidth (m_ngwa_bandwidth);
      return std::min (awnd, m_ngwa_avgbandwidth);
      break;
    default:
      NS_ABORT_MSG ("NGWA-MODE: Fatal, NGWA 'am' is invalid.");
      break;
    }
}
...
{
  if (Node::GlobalFixedTcpWindowSize () != 0)
    {
      NS_LOG_LOGIC ("Using Fixed TCP Window Size");

      return Node::GlobalFixedTcpWindowSize ();
    }

  uint32_t max = 0xffff;
  uint16_t awnd = std::min (m_rxBuffer.MaxBufferSize () - m_rxBuffer.Size (), max);

  if (m_ngwa_bandwidth != 0)
    {
      NS_ABORT_MSG_IF (Node::UseNGWA () == 0, "NGWA-MODE: You enable NGWA, but forgot to set 'ngwa_mode_num'.");

      // Window size have 16 bits. NGWA vars are of 32 bits, converting 32 bits to 16 bits
      // data loss will occour, but we assegure that NGWA 32 bits vars will have maximum 
      // of 16 bits (0xffff), so the convertion will be secure.
      return (uint16_t) DoNGWAModeWindowSize (Node::UseNGWA ());
    }
  else
    {
      return awnd;
