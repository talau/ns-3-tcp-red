...
public:
  ...
  virtual uint32_t GetTripleAcks();
...
protected:
  ...
  // temporary here
  uint32_t DoNGWAModeWindowSize (int mode); // do general action for modes
  void NGWA_filter_bandwidth (uint32_t w); // apply low-pass filter in ngwa
...
protected:
  ...
  SequenceNumber32 m_nextTxSequence; //< Next seqnum to be sent (SND.NXT), ReTx pushes it back
  SequenceNumber32 m_highTxMark;     //< Highest seqno ever sent, regardless of ReTx
  ...
  // temporary place...
  // NGWA
  uint32_t m_ngwa_bandwidth;
  uint32_t m_ngwa_avgbandwidth;

  // Literal names of TCP states for use in log messages */
  static const char* const TcpStateName[LAST_STATE];

  TracedValue<uint32_t>  m_allowedWnd;         //< the effective window used by transmitter
  uint16_t m_fixedTcpWindowSize; // to be compatible with ns-2 and run RED tests
};
