  ...
  static TypeId tid = TypeId ("ns3::DropTailQueue")
    ...
    .AddAttribute ("Mode", 
                   ...
                   MakeEnumChecker (BYTES, "Bytes",
                                    PACKETS, "Packets"))
    .AddAttribute ("Debug", 
                   "If != 0, mark and debug this queue.",
                   UintegerValue (0),
                   MakeUintegerAccessor (&DropTailQueue::m_qdebug),
...
DropTailQueue::DoEnqueue (Ptr<Packet> p)
{
  ...
  //std::cout << "size of packet: " << p->GetSize() << std::endl;
  ...
  if (m_qdebug)
    {
      std::cout << "MAX_PACKETS: " << m_maxPackets << std::endl;
      std::cout << "\tqsize " << m_packets.size () << std::endl;
      std::cout << "\tbytes in queue " << m_bytesInQueue << std::endl;
    }
