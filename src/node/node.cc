...
GlobalValue g_checksumEnabled  = GlobalValue ("ChecksumEnabled",
                                              ...
                                              BooleanValue (false),
                                              MakeBooleanChecker ());
GlobalValue g_IPOptionsEnabled  = GlobalValue ("IPOptionsEnabled",
                                              "A global switch to enable option field in the IP protocol",
                                              BooleanValue (false),
                                              MakeBooleanChecker ());

#define NGWA_MODE0 0 // disable NGWA
#define NGWA_MODE1 1
#define NGWA_MODE2 2
#define NGWA_MODE3 3
#define NGWA_MODE4 4
...
#define NGWA_MODE7 7
#define NGWA_MODE8 8
GlobalValue g_UseNGWA  = GlobalValue ("UseNGWA",
                                              "A global switch to enable NGWA system",
                                              EnumValue (0),
                                      MakeEnumChecker (NGWA_MODE0, "off", NGWA_MODE1, "#1",
                                                       NGWA_MODE2, "#2", NGWA_MODE3, "#3",
                                                       NGWA_MODE4, "#4", NGWA_MODE5, "#5", NGWA_MODE6, "#6",
                                                       NGWA_MODE7, "#7", NGWA_MODE8, "#8"));

GlobalValue g_FixedTcpWindowSize  = GlobalValue ("GlobalFixedTcpWindowSize",
                                                 "A global to set TCP fixed window size",
                                                 //                                                 UintegerValue (0),
                                                 IntegerValue (0),
...
bool
Node::IPOptionsEnabled (void)
{
  BooleanValue val;
  g_IPOptionsEnabled.GetValue (val);
  return val.Get ();
}
...
{
  EnumValue val;
  g_UseNGWA.GetValue (val);
  return val.Get ();
}

uint16_t
Node::GlobalFixedTcpWindowSize (void)
{
  IntegerValue val;

  g_FixedTcpWindowSize.GetValue (val);
  return val.Get ();
}
