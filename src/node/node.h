...
public:
  ...
  /**
   * \returns true if options field of IP procotol are enabled, false otherwise.
   */
  static bool IPOptionsEnabled (void);

  /**
   * \returns the NGWA mode
   */
  static int UseNGWA (void);

  // gambi temp
  // second; number of triples
  std::map<uint16_t, uint16_t> reportTripleAcks;

  static uint16_t GlobalFixedTcpWindowSize (void);
